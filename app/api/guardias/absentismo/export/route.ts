...
import { buildAbsentismoWorkbook, safeFileName } from "@/lib/exportWorkbooks";
...
export async function GET(req: NextRequest) {
  ...
  const workbook = await buildAbsentismoWorkbook(schoolId);
  ...
  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="Absentismo_${safeFileName(school.name)}_${fecha}.xlsx"`,
    },
  });
}
