...
import { buildTutoriasWorkbook, safeFileName } from "@/lib/exportWorkbooks";
...
export async function GET(req: NextRequest) {
  ...
  const { workbook, profesorNombre } = await buildTutoriasWorkbook(schoolId, profesorIdParam);
  ...
  return new NextResponse(buffer, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="Tutorias_${safeFileName(school.name)}${sufijoProfesor}_${fecha}.xlsx"`,
    },
  });
}
